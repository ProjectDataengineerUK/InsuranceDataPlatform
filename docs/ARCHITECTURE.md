...
**Databricks App (Streamlit) com bootstrap de acesso em 2 passos:** cada Databricks App recebe um service principal dedicado, criado automaticamente só no primeiro deploy — não dá pra conceder grants a ele antes disso existir. Fluxo: (1) `databricks bundle deploy -t prod` cria o app (`apps/insurance_platform_app/`) e minta o service principal; (2) o `client_id` gerado é setado em `TF_VAR_app_service_principal_id` e um novo `terraform apply` (via `terraform/unity_catalog.tf::gold_read_visualization_app`/`monitoring_read_visualization_app`) concede `SELECT` nos schemas `gold`/`monitoring` do catálogo prod. Mesma classe de passo manual único já aceita neste repo pro catálogo Unity Catalog (criado pela UI, depois `terraform import`) — não é uma exceção nova ao padrão do projeto.
...
**Limitações herdadas, não escondidas:** a tela "Consultar Cliente" existe no menu (Success Criteria do DEFINE exige as 8 telas navegáveis), mas `customer_id` é sempre nulo em `gold.claims` (limitação já documentada acima) — a página avisa isso explicitamente em vez de fingir que a busca funciona.
...
- Insurance Visualization Layer: dashboard e Genie space ainda não foram prototipados na UI nem capturados via `bundle generate` (só o Databricks App está com código pronto); o bootstrap de 2 passos do grant do service principal do app ainda não foi executado contra um workspace real; página "Lineage" usa um formato de URL do Catalog Explorer não confirmado contra o workspace de produção.
