...
**"`gold.claims` com 0 linhas" era Row Level Security, não pipeline quebrado (mesmo dia):** depois do fix de `schedule` acima, o job `fraud_score_stream` já reportava escritas reais (804, depois 71.866 linhas), mas `SELECT count(*) FROM gold.claims` continuava retornando 0 pro usuário — inclusive pro Databricks App. Causa: `gold_aggregate.py::apply_governance()` reaplica um Row Filter em `gold.claims` a cada execução (`WHEN is_account_group_member('insurance-data-team') THEN true ... ELSE false`), e o grupo `insurance-data-team` não existia ainda neste workspace novo (mesma categoria dos problemas anteriores com `account users`) — sem ele, a condição nunca é verdadeira, e **todo mundo** (usuário e service principal do app) vê 0 linhas, mesmo com os dados fisicamente na tabela. Resolvido criando o grupo `insurance-data-team` no Account Console e adicionando o usuário como membro — comportamento sempre foi "seguro por padrão", não um bug de código.
