...
**`standardize.py` usa `try_cast`, não `cast`, pros campos de valor/data (mesmo dia, 2026-07-13):** o fix acima impede `"NaN"` novo de entrar no Bronze, mas não limpa linhas **já gravadas** antes do fix — e este job sempre relê `bronze.regulatory_claims_raw` inteira a cada execução (sem streaming/checkpoint incremental), então uma linha poluída antiga derrubaria o job pra sempre. Trocado `cast`/`.cast(...)` por `try_cast(...)` (via `expr`) em `valor_indenizacao`/`CLAIM_AMOUNT`/`amountCents`/`occurrenceDate` — entrada malformada agora vira `NULL` (capturado por `check_not_null` como falha de DQ) em vez de derrubar o job inteiro.
